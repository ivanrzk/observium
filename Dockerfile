...
ARG OBSERVIUM_ADMIN_USER
ARG OBSERVIUM_ADMIN_PASS
ARG OBSERVIUM_DB_HOST
ARG OBSERVIUM_DB_USER
ARG OBSERVIUM_DB_PASS
ARG OBSERVIUM_DB_NAME
ARG OBSERVIUM_BASE_URL
ARG OBSERVIUM_SNMP_COMMUNITY
...
ENV OBSERVIUM_SNMP_COMMUNITY=$OBSERVIUM_SNMP_COMMUNITY

# install prerequisites and cleanup
RUN echo 'debconf debconf/frontend select Noninteractive' | debconf-set-selections
RUN apt-get update && \
    apt-get install -y libapache2-mod-php7.4 php7.4-cli php7.4-mysql php7.4-mysqli php7.4-gd php7.4-json php-pear snmp fping mysql-server mysql-client python3-mysqldb rrdtool subversion whois mtr-tiny ipmitool graphviz imagemagick apache2 python3-pymysql python-is-python3 && \
...
RUN cd /opt/observium && \
    ...
    sed -i -e "s/= array(\"public\");/= getenv('OBSERVIUM_SNMP_COMMUNITY');/g" config.php && \
    echo "\$config['base_url'] = getenv('OBSERVIUM_BASE_URL');" >> config.php && \
    echo "\$config['bad_if'] = \"docker\";" >> config.php && \
    echo "\$config['bad_if'] = \"br-\";" >> config.php

COPY build/observium-init.sh /opt/observium/observium-init.sh
...
COPY build/observium-apache24 /etc/apache2/sites-available/000-default.conf
...
COPY build/logrotate-conf /etc/logrotate.conf
COPY build/logrotate-cron /etc/cron.d/logrotate
COPY build/observium-cron /etc/cron.d/observium
